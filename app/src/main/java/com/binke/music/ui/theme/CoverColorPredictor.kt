 ...
 *
 * 纯 Kotlin/Android 原生 API 版 (TFLite 2.8.0, minSdk 19)，
 * 不依赖 androidx.compose.*，ColorTriple 用 ARGB Int 表示。
 */
class CoverColorPredictor(private val context: Context) {

    /**
     * ARGB Int 颜色三元组 (去 Compose 化，Android 4.4 兼容)。
     * - bg: 氛围背景 (HSV 暗 30% 防止太亮刺眼)
     * - pl: 当前歌词 (高亮)
     * - nl: 非当前歌词
     *
     * 调用方直接给 View setBackgroundColor(bg) / setTextColor(pl/nl) 即可。
     */
    data class ColorTriple(val bg: Int, val pl: Int, val nl: Int)
    ...
    /** asset 复制到 cacheDir (TFLite 2.x 限制) */
    ...
    /**
     * 9 维向量 → ARGB Int ColorTriple
     *  bg = out[0..2]  氛围背景 (HSV 暗 30% 防止太亮刺眼)
     *  pl = out[3..5]  当前歌词 (高亮)
     *  nl = out[6..8]  非当前歌词
     */
    private fun toColorTriple(v: FloatArray): ColorTriple {
        val bg = darkenIfVivid(rgbToArgb(v[0], v[1], v[2]), 0.7f)
        val pl = rgbToArgb(v[3], v[4], v[5])
        val nl = rgbToArgb(v[6], v[7], v[8])
        return ColorTriple(bg = bg, pl = pl, nl = nl)
    }

    /**
     * RGB (0..1) → ARGB Int (alpha = 0xFF)。
     * 用 0xFF000000.toInt() or rgb 拼出完整 ARGB。
     */
    private fun rgbToArgb(r: Float, g: Float, b: Float): Int {
     ...
     * 输入输出都是 ARGB Int。
     */
    private fun darkenIfVivid(argb: Int, factor: Float): Int {
        ...
        return if (s > 0.5f && v > 0.5f) {
            val ri = (r * factor * 255f).toInt().coerceIn(0, 255)
            val gi = (g * factor * 255f).toInt().coerceIn(0, 255)
            val bi = (b * factor * 255f).toInt().coerceIn(0, 255)
            0xFF000000.toInt() or (ri shl 16) or (gi shl 8) or bi
        } else {
            argb
        }
    }
